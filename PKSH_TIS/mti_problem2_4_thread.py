...
class MyProblem2(ea.Problem):
    def __init__(self, M=4, pooltype='Thread', num=4):
        name = 'MyProblem2'
        maxormins = [1] * M
...
def subAimFunc(args):
    ...
    if len(set_lst) != len(lst):
        CV_i = np.sum(i) - 2.0
        return [1000], [1000], [1000], [1000], [CV_i.tolist()]
    _i, _r, _m, _mm = util.mti2_four(x1, x2, i, num)

    CV_i = np.sum(i) - 2.0

    return [_i], [_r], [_m], [_mm], [CV_i.tolist()]
